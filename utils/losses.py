...
def logarithm(R, b_deal_with_sym=False, EPS=1e-2):
    ...
    trace_temp = trace_temp.clip(-o_n_e, o_n_e)
    ...
    if b_deal_with_sym:
        log_R = torch.unsqueeze(torch.unsqueeze(mul_expand, 2), 3).to(device) * ss
    else:
        log_R = torch.unsqueeze(torch.unsqueeze(mul_expand, 1), 2).to(device) * ss
...
def get_loss(end_points):
    ...
    trans_loss, trans_perLoss = get_translation_error(translate_pred, translate_label)
    axag_loss, axag_perLoss = get_rotation_error(axag_pred, axag_label)
    ...
    end_points["trans_clsLoss"] = trans_clsLoss
    end_points["axag_clsLoss"] = axag_clsLoss
    end_points["total_clsLoss"] = total_clsLoss

    return total_loss, end_points


if __name__ == "__main__":
    label = torch.tensor([[0.6977, 0.8248, 0.9367]], dtype=torch.float64)
    pred = torch.tensor([[-2.100418, -2.167796, 0.2733]], dtype=torch.float64)
